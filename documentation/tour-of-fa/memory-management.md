...
The `Arena` collection is a special collection optimized to store unordered objects. It's very similar to a `Set`, but without the unicity constraint.

```ts
arena = Arena()

arena.add(Human("John", 20))
arena.add(Human("Jane", 21))
arena.add(Human("John", 22))

for arena >> (human) {
  console::log(human.name) // will log "John", "Jane", "John" -- order is not guaranteed
}

arena.delete(1) // delete the element at index 1
```
